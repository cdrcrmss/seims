...
@php
    ...
    $disposeFormId = 'dispose-unit-form-' . $unit->id;
@endphp
...
<div class="flex flex-wrap items-center justify-end gap-2 shrink-0 sm:ml-4">
    ...
    @if(in_array($unit->status, ['damaged', 'maintenance', 'needs_repair'], true))
        <form method="POST" action="{{ route('maintenance.units.dispose', $unit) }}" id="{{ $disposeFormId }}" class="hidden">
            @csrf
            @method('PATCH')
        </form>
        <button type="button"
                @click="$dispatch('open-confirm-modal', {
                    ...
                    form: document.getElementById(@js($disposeFormId))
                })"
