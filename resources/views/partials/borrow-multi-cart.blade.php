...
<div class="bg-white rounded-2xl ring-1 ring-gray-200 shadow-sm sticky top-6 overflow-hidden">
    ...
    <form method="POST" action="{{ $formAction }}" @submit="handleSubmit($event)" class="p-6 space-y-5">
                ...
                <template x-for="line in cart" :key="line.id">
                    <div class="bg-green-50 ring-1 ring-green-200 rounded-xl p-3">
                        <div class="flex items-start gap-3">
                            ...
                            <button type="button" @click="removeFromCart(line.id)" class="p-1 text-green-400 hover:text-red-500 hover:bg-red-50 rounded-lg">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                            </button>
                        </div>
                        <div class="mt-3 flex items-center justify-end gap-2">
                            <span class="text-sm font-bold text-gray-900 min-w-[1.5rem] text-center" x-text="line.quantity"></span>
                            <div class="flex flex-col border border-green-200 rounded-md bg-white overflow-hidden divide-y divide-green-100">
                                <button type="button" @click="incrementQty(line.id)" :disabled="line.quantity >= line.stock"
                                        class="px-2 py-0.5 text-gray-500 hover:bg-gray-50 disabled:opacity-40" aria-label="Increase quantity">
                                    <svg class="w-3 h-3 mx-auto" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M14.707 12.707a1 1 0 01-1.414 0L10 9.414l-3.293 3.293a1 1 0 01-1.414-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 010 1.414z" clip-rule="evenodd"/></svg>
                                </button>
                                <button type="button" @click="decrementQty(line.id)" :disabled="line.quantity <= 1"
                                        class="px-2 py-0.5 text-gray-500 hover:bg-gray-50 disabled:opacity-40" aria-label="Decrease quantity">
                                    <svg class="w-3 h-3 mx-auto" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd"/></svg>
                                </button>
                            </div>
                        </div>
                    </div>
        ...
        <div>
            ...
            <p class="text-[11px] text-gray-400 mt-1">{{ $returnHoursHint ?? 'Choose return time: 3–8 hours' }}</p>
        ...
        <div class="bg-gray-50 rounded-xl p-4 space-y-2">
            <div class="flex justify-between items-center">
                <span class="text-xs text-gray-500">Borrow window</span>
        ...
        <div class="flex gap-3 pt-1">
            <button type="submit" :disabled="isSubmitting || !canSubmit"
                    class="flex-1 py-3 text-white font-semibold rounded-xl text-sm transition-all flex items-center justify-center gap-2"
                    :class="(isSubmitting || !canSubmit) ? 'bg-gray-300 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700 shadow-sm'">
                <template x-if="isSubmitting">
                    <svg class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
                </template>
                <span x-text="isSubmitting ? 'Processing...' : @json($submitLabel)"></span>
            </button>
