...
<div class="flex items-center space-x-3 {{ $s['bg'] }} rounded-xl p-4 ring-1 {{ $s['ring'] }}" role="alert">
    <div class="w-9 h-9 {{ $s['icon_bg'] }} rounded-lg flex items-center justify-center flex-shrink-0" aria-hidden="true">
        <svg class="w-4 h-4 {{ $s['icon'] }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
        </svg>
    </div>
    <div class="flex-1 min-w-0">
        <p class="text-sm font-semibold {{ $s['text'] }}">{{ $message }}</p>
    </div>
    @if($actionUrl)
    <a href="{{ $actionUrl }}" class="text-xs font-semibold {{ $s['link'] }} whitespace-nowrap">{{ $actionLabel }} &rarr;</a>
    @endif
</div>
