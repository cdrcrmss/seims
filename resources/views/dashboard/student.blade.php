...
<div class="space-y-6" x-data="{ showBorrowModal: false, selectedItem: null }">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
        <a href="{{ route('student.borrowings.index', ['status' => 'issued']) }}" class="bg-white rounded-2xl p-6 ring-1 ring-gray-100 shadow-sm card-hover relative overflow-hidden cursor-pointer hover:ring-green-200 transition-all">
        ...
        <a href="{{ route('student.borrow.form') }}" class="bg-white rounded-2xl p-6 ring-1 ring-gray-100 shadow-sm card-hover relative overflow-hidden cursor-pointer hover:ring-blue-200 transition-all">
        ...
        <a href="{{ route('student.borrowings.index', ['status' => 'returned']) }}" class="bg-white rounded-2xl p-6 ring-1 ring-gray-100 shadow-sm card-hover relative overflow-hidden cursor-pointer hover:ring-gray-200 transition-all">
            ...
            <div class="px-5 py-3 hover:bg-gray-50 transition-colors flex items-center justify-between gap-4">
                <div class="flex items-center space-x-3 min-w-0">
                    <div class="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center flex-shrink-0">
                        @if($borrowing->item?->image_url)
                            <img src="{{ $borrowing->item->image_url }}" class="w-8 h-8 rounded-lg object-cover" alt="">
                        @else
