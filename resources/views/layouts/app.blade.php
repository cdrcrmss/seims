<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" x-data="{ darkMode: localStorage.getItem('darkMode') === 'true', sidebarOpen: false }" x-init="$watch('darkMode', val => localStorage.setItem('darkMode', val))" :class="{ 'dark': darkMode }" style="background-color: #f9fafb;">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="csrf-token" content="{{ csrf_token() }}">

    <title>{{ config('app.name', 'INNOTRACK') }} - @yield('title', 'Laboratory Management')</title>
                ...
                <div class="flex justify-between h-16">
                    ...
                    <div class="flex items-center">
                        <div class="flex-shrink-0 flex items-center space-x-3">
                            <img src="{{ asset('images/spup_logo.png') }}" alt="SPUP Logo" class="w-10 h-10 object-contain">
                    ...
                    <div class="hidden sm:ml-6 sm:flex sm:items-center sm:space-x-8">
                        <a href="{{ route('dashboard') }}" class="group relative px-3 py-2 text-sm font-medium text-gray-900 dark:text-gray-100 hover:text-green-600 dark:hover:text-green-400 transition-all duration-200">
                        ...
                        @if(auth()->user()->isStaff() || auth()->user()->isAdmin())
                            <a href="{{ route('staff.items.index') }}" class="group relative px-3 py-2 text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-green-600 dark:hover:text-green-400 transition-all duration-200">
                                <span class="relative z-10">Manage Items</span>
                                <div class="absolute inset-0 bg-green-500/10 rounded-lg scale-0 group-hover:scale-100 transition-transform duration-200"></div>
                            </a>
                            <a href="{{ route('staff.borrowings.index') }}" class="group relative px-3 py-2 text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-green-600 dark:hover:text-green-400 transition-all duration-200">
                                <span class="relative z-10">Requests</span>
                                <div class="absolute inset-0 bg-green-500/10 rounded-lg scale-0 group-hover:scale-100 transition-transform duration-200"></div>
                            </a>
                        @endif
                        @if(auth()->user()->isAdmin())
                            <a href="{{ route('admin.users.index') }}" class="group relative px-3 py-2 text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-green-600 dark:hover:text-green-400 transition-all duration-200">
                        ...
                        <div x-data="{ open: false }" class="relative">
                            ...
                            <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="absolute right-0 mt-2 w-56 bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl rounded-2xl shadow-xl border border-gray-200/20 dark:border-gray-700/30 py-2">
                                <div class="px-4 py-3 border-b border-gray-200/20 dark:border-gray-700/30">
                                    <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ auth()->user()->name }}</p>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 capitalize">{{ auth()->user()->email }}</p>
                                </div>
                ...
                <div class="fixed inset-y-0 left-0 z-50 w-64 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-r border-gray-200/20 dark:border-gray-700/30" x-show="sidebarOpen" x-transition:enter="transition ease-in-out duration-300 transform" x-transition:enter-start="-translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in-out duration-300 transform" x-transition:leave-start="translate-x-0" x-transition:leave-end="-translate-x-full">
                    <div class="flex items-center justify-between px-4 py-4 border-b border-gray-200/20 dark:border-gray-700/30">
                        <div class="flex items-center space-x-2">
                            <img src="{{ asset('images/spup_logo.png') }}" alt="SPUP Logo" class="w-8 h-8 object-contain">
                    ...
                    <div class="px-4 py-6 space-y-2">
                        <a href="{{ route('dashboard') }}" class="flex items-center px-3 py-2 rounded-lg text-gray-900 dark:text-gray-100 hover:bg-green-500/10 transition-colors">
                        ...
                        @if(auth()->user()->isStaff() || auth()->user()->isAdmin())
                            <a href="{{ route('staff.items.index') }}" class="flex items-center px-3 py-2 rounded-lg text-gray-500 dark:text-gray-400 hover:bg-green-500/10 transition-colors">
                                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path>
                                </svg>
                                Manage Items
                            </a>
                            <a href="{{ route('staff.borrowings.index') }}" class="flex items-center px-3 py-2 rounded-lg text-gray-500 dark:text-gray-400 hover:bg-green-500/10 transition-colors">
                                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @endif
                        @if(auth()->user()->isAdmin())
                            <a href="{{ route('admin.users.index') }}" class="flex items-center px-3 py-2 rounded-lg text-gray-500 dark:text-gray-400 hover:bg-green-500/10 transition-colors">
                                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
