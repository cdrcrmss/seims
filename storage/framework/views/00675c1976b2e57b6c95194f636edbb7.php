...
<div class="space-y-6" x-data="{ 
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 animate-fade-in-up">
        ...
        <div class="flex items-center space-x-3">
            ...
            <a href="<?php echo e(route('reservations.create')); ?>" class="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-xl text-sm font-semibold shadow-sm hover:shadow-md transition-all duration-200">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="text-gray-900 text-xs">
                                <p><?php echo e(\Carbon\Carbon::parse($reservation->start_datetime)->format('M d, Y g:ia')); ?></p>
                                <p class="text-gray-500">to <?php echo e(\Carbon\Carbon::parse($reservation->end_datetime)->format('M d, Y g:ia')); ?></p>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <p class="text-gray-700 truncate max-w-[200px]"><?php echo e($reservation->purpose); ?></p>
                        </td>
                        <td class="px-6 py-4">
                            <?php
                                $statusColors = [
                            ...
                            <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold <?php echo e($statusColors[$reservation->status] ?? 'bg-gray-100 text-gray-700'); ?>">
                                <?php if($reservation->conflict_detected): ?>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end space-x-2">
                                ...
                                <form method="POST" action="<?php echo e(route('reservations.reject', $reservation)); ?>" x-data @submit.prevent="$dispatch('open-confirm-modal', { form: $el, title: 'Reject Reservation', message: 'Are you sure you want to reject this reservation request?', type: 'danger' })">
                                    <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                                ...
                                <?php if($reservation->status === 'pending' || ($reservation->status === 'approved' && (auth()->id() === $reservation->user_id || in_array(auth()->user()->role, ['staff', 'admin'])))): ?>
                                <button @click="showCancelModal = true; cancelId = <?php echo e($reservation->id); ?>" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg bg-red-50 text-red-700 hover:bg-red-100 ring-1 ring-red-200/60 transition-all duration-200">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                                    Cancel
                                </button>
                                <?php endif; ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Cedric\SEIMS\resources\views/reservations/index.blade.php ENDPATH**/ ?>