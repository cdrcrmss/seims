<?php $__env->startSection('title', 'Trash - Item Management'); ?>
...
<div class="space-y-8" x-data="trashSearch()" x-init="initSearch()">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div>
            <a href="<?php echo e(route('staff.items.index')); ?>" class="inline-flex items-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-700 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200">
    ...
    <div class="bg-white rounded-2xl ring-1 ring-gray-200 shadow-sm p-6 relative">
        <form method="GET" class="flex flex-col sm:flex-row gap-4">
            ...
            <a href="<?php echo e(route('staff.items.trash')); ?>" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-6 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 text-center">
                Clear
            </a>
        </form>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50">
                    <?php $__empty_1 = true; $__currentLoopData = $trashedItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        <td class="px-6 py-4">
                            <div class="flex items-center space-x-3">
                                <?php if($item->image_path): ?>
                                    <img src="<?php echo e(asset('storage/' . $item->image_path)); ?>" class="w-10 h-10 rounded-lg object-cover" alt="">
                                <?php else: ?>
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-400"><?php echo e($item->qr_code ?? 'N/A'); ?></p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-gray-700"><?php echo e($item->category); ?></td>
                        <td class="px-6 py-4 text-gray-700 font-mono text-xs"><?php echo e($item->asset_code ?? 'N/A'); ?></td>
                        <td class="px-6 py-4">
                            <div class="flex items-center space-x-2">
                                <span class="text-gray-700"><?php echo e($item->available_stock); ?> / <?php echo e($item->total_stock); ?></span>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-gray-500 text-xs"><?php echo e($item->deleted_at->format('M d, Y - g:i A')); ?></td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end space-x-2">
                                <form method="POST" action="<?php echo e(route('staff.items.restore', $item->id)); ?>" class="inline" x-data @submit.prevent="$dispatch('open-confirm-modal', { form: $el, title: 'Restore Item', message: 'Are you sure you want to restore this item?', type: 'success' })">
                                    <?php echo csrf_field(); ?>
                                ...
                                <form method="POST" action="<?php echo e(route('staff.items.force-delete', $item->id)); ?>" class="inline" x-data @submit.prevent="$dispatch('open-confirm-modal', { form: $el, title: 'Permanently Delete', message: 'Are you sure you want to permanently delete this item? This action cannot be undone.', type: 'danger' })">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Cedric\SEIMS\resources\views/staff/items/trash.blade.php ENDPATH**/ ?>