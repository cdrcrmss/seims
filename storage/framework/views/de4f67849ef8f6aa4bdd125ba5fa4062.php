...
<div class="space-y-8" x-data="{ showAddItemModal: false, showImportModal: false, showQrModal: false, qrItem: null, showUnitsModal: false, unitsData: { item_name: '', units: [], total: 0 }, unitsLoading: false, async loadUnits(itemId) { this.unitsLoading = true; this.showUnitsModal = true; try { const res = await fetch('/staff/items/' + itemId + '/units'); this.unitsData = await res.json(); } catch(e) { this.unitsData = { item_name: 'Error', units: [], total: 0 }; } this.unitsLoading = false; this.$nextTick(() => { setTimeout(() => { this.unitsData.units.forEach(unit => { generateUnitQr(unit.id, unit.qr_code); }); }, 150); }); } }">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <a href="<?php echo e(route('staff.items.index')); ?>" class="bg-white rounded-2xl p-6 ring-1 ring-gray-100 shadow-sm card-hover relative overflow-hidden cursor-pointer hover:ring-green-200 transition-all">
        ...
        <a href="<?php echo e(route('staff.items.index', ['status' => 'available'])); ?>" class="bg-white rounded-2xl p-6 ring-1 ring-gray-100 shadow-sm card-hover relative overflow-hidden cursor-pointer hover:ring-emerald-200 transition-all">
        ...
        <a href="<?php echo e(route('staff.items.index', ['status' => 'out_of_stock'])); ?>" class="bg-white rounded-2xl p-6 ring-1 ring-gray-100 shadow-sm card-hover relative overflow-hidden cursor-pointer hover:ring-red-200 transition-all">
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    <?php $__empty_1 = true; $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-gray-50 transition-colors item-row" data-search="<?php echo e(strtolower($item->name . ' ' . ($item->description ?? '') . ' ' . $item->category . ' ' . $item->status)); ?>">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center gap-1">
                                    <button type="button" @click="loadUnits(<?php echo e($item->id); ?>)" class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 transition-all duration-200" title="View Units & QR Codes (<?php echo e($item->total_stock); ?>)">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"></path></svg>
                                    </button>
    ...
    <div x-show="showImportModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto"
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div class="relative z-10 w-full max-w-lg bg-white rounded-3xl shadow-2xl transform transition-all"
                ...
                <form method="POST" action="<?php echo e(route('staff.items.bulk-import')); ?>" enctype="multipart/form-data" class="p-6">
                    <?php echo csrf_field(); ?>
                    <div class="space-y-4">
