...
<div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 animate-fade-in-up">
        ...
        <div class="flex space-x-3">
            <a href="<?php echo e(route('maintenance.dashboard')); ?>" class="inline-flex items-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-700 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>
                Dashboard
            </a>
            <a href="<?php echo e(route('maintenance.create')); ?>" class="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-xl text-sm font-semibold shadow-sm hover:shadow-md transition-all duration-200">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50">
                    <?php $__empty_1 = true; $__currentLoopData = $maintenanceRecords; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $record): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        <td class="px-6 py-4 font-medium text-gray-900"><?php echo e($record->item?->name ?? 'N/A'); ?></td>
                        <td class="px-6 py-4">
                            <?php
                                $typeColors = [
                            ...
                            ?>
                            <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold <?php echo e($typeColors[$record->maintenance_type] ?? 'bg-gray-100 text-gray-700'); ?>">
                                <?php if($record->predictive_alert_sent): ?>
                                    <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2L1 21h22L12 2zm0 4l7.53 13H4.47L12 6z"/></svg>
                                <?php endif; ?>
                                <?php echo e(ucfirst($record->maintenance_type)); ?>

                            </span>
                        </td>
                        <td class="px-6 py-4 text-gray-700">
                            <?php echo e($record->scheduled_date ? $record->scheduled_date->format('M d, Y') : 'N/A'); ?>

                            <?php if($record->status === 'scheduled' && $record->scheduled_date && $record->scheduled_date->isPast()): ?>
                                <span class="text-red-500 text-xs font-semibold block">OVERDUE</span>
                            <?php endif; ?>
                        </td>
                        <td class="px-6 py-4 text-gray-700"><?php echo e($record->technician->name ?? 'Unassigned'); ?></td>
                        <td class="px-6 py-4">
                            <?php if($record->wear_level !== null): ?>
                            <div class="flex items-center space-x-2">
                                <div class="w-16 bg-gray-200 rounded-full h-2">
                                    <div class="h-2 rounded-full <?php echo e($record->wear_level >= 70 ? 'bg-red-500' : ($record->wear_level >= 40 ? 'bg-yellow-500' : 'bg-green-500')); ?>" style="width: <?php echo e($record->wear_level); ?>%"></div>
                                </div>
                                <span class="text-xs text-gray-600"><?php echo e($record->wear_level); ?>%</span>
                            </div>
                            <?php else: ?>
                        ...
                        <td class="px-6 py-4">
                            ...
                            ?>
                            <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold <?php echo e($statusColors[$record->status] ?? 'bg-gray-100 text-gray-700'); ?>">
                        ...
                        <td class="px-6 py-4 text-right">
                            ...
                            <button @click="completeModal = true; completeId = <?php echo e($record->id); ?>" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg bg-green-50 text-green-700 hover:bg-green-100 ring-1 ring-green-200/60 transition-all duration-200">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Cedric\SEIMS\resources\views/maintenance/index.blade.php ENDPATH**/ ?>